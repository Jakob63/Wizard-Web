    ...
    <div id="menu">
      <div class="container">
        ...
        <div class="player-count-row">
          <button v-for="n in 4" :key="n+2" type="button" class="player-btn" :class="{active: selectedCount===n+2}" @click="selectCount(n+2)">
            {{ n+2 }}
          </button>
        </div>
        ...
        <div id="nameForm">
          ...
          <div id="namesError" class="text-danger" v-show="errorMsg">{{ errorMsg }}</div>

          <div class="mt-3 d-flex gap-2">
            <button type="button" class="btn btn-primary" @click="startGame" :disabled="submitting">Spiel starten</button>
  ...
  methods: {
    selectCount(n) {
      if (n >= 3 && n <= 6) {
        ...
        this.$nextTick(() => {
          const firstEmpty = Array.from(document.querySelectorAll('#nameForm input[id^="name"]'))
              .find(i => !i.value) || document.querySelector('#nameForm input[id^="name"]');
          firstEmpty?.focus();
        });
      }
    },
    onPresetChange(e) {
      ...
      apiGet(`/pwa/api/playerPresets/${encodeURIComponent(id)}`)
          .then(data => {
            ...
            if (players.length >= 3 && players.length <= 6) this.selectedCount = players.length;
            this.$nextTick(() => {
    ...
    async startGame() {
      ...
      try {
        ...
        const offline = (e?.name === 'TypeError') || (e?.status === 0) || m.includes('failed to fetch') || !navigator.onLine;
        if (offline) {
