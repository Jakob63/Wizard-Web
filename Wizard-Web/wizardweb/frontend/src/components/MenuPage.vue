...
export default {
  ...
  mounted(){
    // Ensure Play's jsRoutes is available for legacy menu.js
    try {
      if (!window.jsRoutes && !document.querySelector('script[data-jsroutes]')) {
        const s = document.createElement('script');
        s.src = '/jsroutes';
        s.setAttribute('data-jsroutes','1');
        document.head.appendChild(s);
      }
    } catch(_) {}

    // Load legacy menu.js which wires up the form events
    try {
      const url = new URL('../../../public/javascripts/menu.js', import.meta.url).href;
